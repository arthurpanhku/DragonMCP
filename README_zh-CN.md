...
| 工具 | 数据源 | 需要 Key | 说明 |
| :--- | :--- | :--- | :--- |
| `search_mtr_schedule` | 港铁 via data.gov.hk | 否 | 两站之间实时到站，**覆盖全部 10 条线** |
...
- **不支持港铁换乘规划。** 仅支持直达线路。两站不在同一条线上时，会明确返回「未找到直达线路」，而不是猜一个答案。
- **东铁线与将军澳线的分支处理是简化的。** 两条线都有分支（罗湖/落马洲、康城/宝琳），目前只取主分支。
